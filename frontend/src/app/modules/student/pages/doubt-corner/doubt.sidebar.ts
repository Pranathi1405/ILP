...
export class DoubtSidebar implements OnInit, OnDestroy {
  ...
  subjects = this.doubtService.subjects;
  ...
  onCourseSelect(courseId: number | null): void {
    ...
    if (courseId !== null) {
      this.doubtService.fetchSubjects(courseId).subscribe();
    } else {
      this.doubtService.subjects.set([]);
    }
    this.loadDoubts();
  }
  ...
  onSubjectSelect(subjectId: number): void {
    ...
    this.selectedSubjectId.set(current === subjectId ? null : subjectId);
    this.loadDoubts();
  }
