...
export class DoubtChat {
  ...
  onResolve(): void {
    const doubt = this.doubtService.activeDoubt();
    if (!doubt || doubt.status === 'RESOLVED') return;
    this.doubtService.resolveDoubt(doubt.doubt_id).subscribe();
  }

  isOwnReply(responderType: string): boolean {
    return responderType === 'student';
  }

  facultyName = computed(() => {
    const detail = this.doubtService.doubtDetail();
    if (!detail) return null;
    const teacherReply = detail.teacher.name;
    return teacherReply ?? null;
  });
